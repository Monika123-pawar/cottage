...
class Bubble {
    constructor(){
        ...
        this.y = canvas.height + 50 + Math.random() * canvas.height/2;
        this.radius = 50;
...
ctx.fillText('Welcome To ',3, 30);
ctx.fillText('Shree',30, 50);
ctx.fillText('Gurukrupa',10, 70);
    ...
    for (let y = 0, y2 = textCoordinates.height; y < y2; y++){
        for (let x = 0, x2 = textCoordinates.width; x < x2; x++){
            if (textCoordinates.data[(y * 4 * textCoordinates.width) + (x * 4) + 3] > 128){
                ...
                bubbleTextArray.push(new Particle2(positionX * 12, positionY * 8));
            }
        }
    }
}
